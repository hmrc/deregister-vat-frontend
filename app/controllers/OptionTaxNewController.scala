...
@Singleton
class OptionTaxNewController @Inject()(optionTax: OptionTaxNew,
                                    val mcc: MessagesControllerComponents,
                                    val authenticate: AuthPredicate,
                                    val regStatusCheck: DeniedAccessPredicate,
                                    val optionTaxAnswerService: OptionTaxNewAnswerService,
                                    val serviceErrorHandler: ServiceErrorHandler,
                                    implicit val ec: ExecutionContext,
                                    implicit val appConfig: AppConfig) extends FrontendController(mcc) with I18nSupport with LoggingUtil{

  val form: Form[YesNo] = YesNoForm.yesNoForm("optionTax.error.mandatoryRadioOption")

  val show: Action[AnyContent] = (authenticate andThen regStatusCheck).async { implicit user =>
    optionTaxAnswerService.getAnswer map {
      case Right(Some(data)) => Ok(optionTax(form.fill(data)))
      case _ => Ok(optionTax(form))
    }
  }
  ...
  val submit: Action[AnyContent] = authenticate.async { implicit user =>
    form.bindFromRequest().fold(
      ...
      answer => optionTaxAnswerService.storeAnswer(answer).flatMap {
        case Right(_) =>
