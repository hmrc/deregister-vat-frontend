...
class AuthoriseAsAgent @Inject()(enrolmentsAuthService: EnrolmentsAuthService,
  ...
  extends FrontendController(mcc) with I18nSupport with ActionBuilder[User, AnyContent] with ActionFunction[Request, User] with LoggingUtil {
  ...
  override def invokeBlock[A](request: Request[A], block: User[A] => Future[Result]): Future[Result] = {
    ...
    request.session.get(SessionKeys.clientVRN) match {
      case Some(vrn) =>
        debug(s"[AuthoriseAsAgent][invokeBlock] - Client VRN from Session: $vrn")
        enrolmentsAuthService
          .authorised(delegatedAuthorityRule(vrn))
          .retrieve(allEnrolments) {
             debug(s"[AuthoriseAsAgent][invokeBlock] - Authenticated as Agent")
            allEnrolments => block(User(vrn, active = true, Some(arn(allEnrolments))))
          } recover {
          case _: NoActiveSession =>
             debug(s"[AuthoriseAsAgent][invokeBlock] - Agent does not have an active session, redirect to GG Sign In")
            Redirect(appConfig.signInUrl)
          case _ =>
             debug(s"[AuthoriseAsAgent][invokeBlock] - Agent does not have delegated authority for Client")
            Redirect(appConfig.agentClientUnauthorisedUrl)
        }
      case _ =>
        infoLog("[AuthoriseAsAgent][invokeBlock] - No Client VRN in session")
