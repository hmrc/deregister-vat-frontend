...
object BusinessActivityForm {
  ...
  val businessActivityError: String = "businessActivity.error"
  ...
  val formatter: Formatter[YesNo] = new Formatter[YesNo] {
    override def bind(key: String, data: Map[String, String]): Either[Seq[FormError], YesNo] = {
      data.get(key) match {
        ...
        case _ => Left(Seq(FormError(key, businessActivityError)))
      }
    }
  ...
  val businessActivityForm: Form[YesNo] = Form(
    single(
      yesNo -> of(formatter)
    )
  )
