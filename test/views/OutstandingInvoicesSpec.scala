...
import assets.messages.{CommonMessages, OutstandingInvoicesMessages}
...
class OutstandingInvoicesSpec extends ViewBaseSpec {
  ...
  "Rendering the option to tax page with errors" should {
    ...
    "have the correct document title" in {
      document.title shouldBe s"${CommonMessages.errorPrefix} ${OutstandingInvoicesMessages.title}"
    }
