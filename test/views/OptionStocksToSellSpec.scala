...
class OptionStocksToSellSpec extends ViewBaseSpec {
  ...
  "Rendering the option to tax page with errors" when {
    ...
    "nothing was entered" should {
      ...
      "have the correct document title" in {
        document.title shouldBe s"${CommonMessages.errorPrefix} ${OptionStocksToSellMessages.title}"
      }
      ...
      "display the correct error messages" in {
        elementText(Selectors.error) shouldBe s"${CommonMessages.errorPrefix} ${OptionStocksToSellMessages.yesNoError}"
      }
    }
