...
class OptionTaxNewSpec extends ViewBaseSpec {
  ...
  "Rendering the option to tax page with no errors" should {
    ...
    lazy val view = optionTax(YesNoForm.yesNoForm("optionTax.error.mandatoryRadioOption"))
  ...
  "Rendering the option to tax page with errors" when {
    ...
    "nothing was entered" should {
      ...
      lazy val view = optionTax(YesNoForm.yesNoForm("optionTax.error.mandatoryRadioOption").bind(Map("yes_no" -> "")))
      lazy implicit val document: Document = Jsoup.parse(view.body)
