...
trait MockNextTaxableTurnoverAnswerService extends MockStoredAnswersService {
  ...
  def setupMockGetNextTaxableTurnover(response: Either[ErrorModel, Option[MonetaryModel]])(implicit user: User[_]): Unit =
    setupMockGetAnswers(mockNextTaxableTurnoverAnswerService)(response)

  def setupMockStoreNextTaxableTurnover(data: MonetaryModel)(response: Either[ErrorModel, DeregisterVatResponse])(implicit user: User[_]): Unit =
